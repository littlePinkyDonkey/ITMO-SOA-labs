...
public class DragonRepository {
    ...
    public int removeElementByCharacter(final DragonCharacter character) throws BusinessException {
        ...
        int updated;
        try {
            updated = session.createQuery("delete from andrei.teplykh.entity.Dragon where character=:character")
    ...
    public Dragon getWithMinId() throws BusinessException {
        ...
        final List<Dragon> dragons;
        try {
            final Query query = session.createQuery("from andrei.teplykh.entity.Dragon d order by id");
            dragons = query.list();
